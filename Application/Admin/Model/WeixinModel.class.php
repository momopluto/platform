...
//			return true;
	...
	/* 发送回复消息到微信平台 */
	public function _replyData($msg, $msgType) {
    ...
    public function subscribe() {
        ...
        $config = M('welcome')->where(array('token' => $param ['token']))->find();
    ...
    // 关键字回复
    public function keyword_handle($key){

        if($key == null){
           return false;
        }

        $result = match_keyword($key);//匹配关键字，返回结果
        if($result){
                        /*
                [0] => Array
                    (
                        [id] => 480
                        [keyword] =>  453 5
                        [token] => gh_34b3b2e6ed7f
                        [from_id] => 32
                        [extra_text] => keyword_reply_text
                        [cTime] => 1406516929
                        [keyword_length] => 3
                        [keyword_type] => 0
                        [sort] => 36
            ...
            $map ['id'] = $result ['from_id'];
            if ($result ['extra_text'] == 'keyword_reply_mult') {
                // 多图文回复
                $mult = M ( 'keyword_reply_mult' )->where ( $map )->find ();
                $map_news ['id'] = array (
                    'in',
                    $mult ['mult_ids']
                );
                $list = M ( 'keyword_reply_news' )->where ( $map_news )->select ();
                ...
                $res = $this->replyNews ( $articles );
            } elseif ($result ['extra_text'] == 'keyword_reply_news') {
                // 单条图文回复
                $info = M ( 'keyword_reply_news' )->where ( $map )->find ();
                ...
                // 文本回复
                $info = M ( 'keyword_reply_text' )->where ( $map )->find ();
                $res = $this->replyText ( htmlspecialchars_decode ( $info ['content'] ) );
                //$res = $this->replyText ( json_encode($this->test_out)  );
            }
            return $res;
        }else{
            $result_text = playing_cute();//卖萌回复
            if($result_text){
                $this->replyText(htmlspecialchars_decode ( $result_text ));
            }            
            return false;
        }
    }
    ...
    //“订单”菜单操作
    public function order_handle($key){
        $param ['token'] = get_token ();
        $param ['openid'] = get_openid ();

        switch($key){
            case '提交订单':
                $result = menu_submit_order($param);
                if(is_string($result)){//字符串
                    $this->replyText ( htmlspecialchars_decode ( $result ) );
                }else{//数组
                    $this->replyNews ( $result );
                }
                break;
            case '清空选单':
                $result = menu_clear_order($param);
                if(is_string($result)){//字符串
                    $this->replyText ( htmlspecialchars_decode ( $result ) );
                }else{//数组
                    $this->replyNews ( $result );
    ...
    //菜单事件处理
    public function menu_handle($key){
        if($key == null){
            return false;
        }
        ...
        $result_text = menu_info_handle($current_sel, $current_op);

        $res = $this->replyText ( htmlspecialchars_decode ( $result_text ) );
        return $res;
    }
